...
START_TEST(pow_54) {
  long double exp = -0;
  ck_assert_ldouble_eq_tol(s21_pow(NAN, exp), pow(NAN, exp), S21_EPS);
}
END_TEST
...
START_TEST(pow_58) {
  long double exp = -INFINITY;
  ck_assert_ldouble_eq_tol(s21_pow(-INFINITY, exp), pow(-INFINITY, exp),
                           S21_EPS);
}
END_TEST
...
START_TEST(pow_60) {
  ...
  for (exp = 0.0; exp > -1000; exp -= 0.1) {
    res1 = pow(INFINITY, exp);
    res2 = s21_pow(INFINITY, exp);
  }
...
START_TEST(pow_61) {
  ...
  for (base = 0.0; base < 10; base += 0.1) {
    res1 = pow(base, INFINITY);
    res2 = s21_pow(base, INFINITY);
  }
...
START_TEST(pow_75) {
  ck_assert_ldouble_eq_tol(s21_pow(-INFINITY, -1), pow(-INFINITY, -1), S21_EPS);
}
END_TEST

Suite *suite_pow(void) {
  Suite *s = suite_create("s21_pow");
  TCase *tc = tcase_create("case_pow");
